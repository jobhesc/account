...
/**
 * 使用sqlcipher加密数据库，方法：下载sqlcipher for android，然后把解压缩后把libs和assets拷贝到项目中覆盖；
 * 然后使用时，SQLiteOpenHelper和SQLiteDatabase引入的类全名为：info.guardianproject.database.sqlcipher.SQLiteOpenHelper
 * 和info.guardianproject.database.sqlcipher.SQLiteDatabase，并且在使用之前，需要SQLiteDatabase.loadLibs(context)方法装载sqlcipher库，
 * 同时dbHelper.getReadableDatabase和dbHelper.getWritableDatabase需要传入加密key值，
 * 其他使用方法与android包的SQLiteOpenHelper和SQLiteDatabase一样；
 * @author hesc
 *
 */
public class SqlcipherImagePersistence implements IImagePersistence {
	public static final String DBNAME="upload2.db";
	private static final String cipherKey = "foo123";
	...
	public SqlcipherImagePersistence(Context context){
		// 装载sqlcipher类库
	...
	private ImageModel[] loadFromDB(String filter) throws Exception {
		...
		Cursor c = null;
		try {
			db = dbHelper.getReadableDatabase(cipherKey);
	...
	private void saveToDB(ImageModel[] models){
		if(models == null || models.length == 0) return;
		SQLiteDatabase db = dbHelper.getWritableDatabase(cipherKey);
		db.beginTransaction();
		try {
