...
import com.ynt.account.request.ReportLrbBean;
...
public class ReportSWBBActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.report_swbb);
		
		reportView = (ReportView)findViewById(R.id.report_swbb_view);
		...
		ReportColumn currMnyColumn = new ReportColumn(this, "本期金额", "currMny", R.style.report_head, R.style.report_body);
		currMnyColumn.getRowStyle().setGravity(Gravity.RIGHT|Gravity.CENTER_VERTICAL);
		reportView.addColumn(currMnyColumn);
		
		ReportColumn totalMnyColumn = new ReportColumn(this, "本年累计", "totalMny", R.style.report_head, R.style.report_body);
		...
		txtPeriodView.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				if(event.getAction() == MotionEvent.ACTION_UP){
					...
					UIHelper.showDatePickerDialog(ReportSWBBActivity.this, dateFormat, txtPeriodView);
				}
				return true;
			}
		});
